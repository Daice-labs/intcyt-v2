...
#-----------------------------------------------------------------  
  def identity(self,vector):
    if len(vector) != self.dimension:
      raise ValueError("Error: in Operad.identity: dimensions do not match") 
  ...
  def tensor(self,list_of_cells):
    for i in range(len(list_of_cells)):
      if self.dimension != list_of_cells[i].dimension:
        raise ValueError("Error: in Operad.tensor: dimensions do not match")
    organelles = [[0] * self.dimension for _ in range(len(list_of_cells))]
  ...
  def pseudo_tensor(self,list_of_super_cells):
    for i in range(len(list_of_super_cells)):
      if self.dimension != list_of_super_cells[i].cell.dimension:
        raise ValueError("Error: in Operad.merging_tensor: dimensions do not match")

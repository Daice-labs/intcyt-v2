...
#-----------------------------------------------------------------
  def allostasis(self,vector,identity,gamma):
    self.pre_action = list()
    def log_average_change(label,matrix,stage): 
      ...
      for row in matrix:
        if row:
          ...
          averages.append(0.0)
      logger.debug("%s [%s] averages=%s rows=%s", label, stage, averages, matrix)
    #~~~~~~~~~~~~~~~~~~~
    if self.is_leaf:
      for i in range(len(self.cell.organelles)):
        self.pre_action.append(vector)
    ...
    log_average_change("Gamma parameters",parameter_gamma,"initial")
    #~~~~~~~~~~~~~~~~~~~      
    ...
    log_average_change("Allostasis matrix",gradient_descent,"pre-homeostasis")
    #~~~~~~~~~~~~~~~~~~~
    self.homeostasis(gradient_descent,identity)
    log_average_change("Allostasis matrix",gradient_descent,"post-homeostasis")
    return self
#------------------------------------------------------------------------------  
  ...
  def _fusion(self,tensor,filtering = [1.5,0]):
    if not(self.is_leaf):
      ...
      if len(mer) in [0,len(self.cell.organelles)]:
        logger.warning("SuperCell._fusion: total fusion canceled at level %s", self.level)
      else:
        self.merge_base(mer,tensor)
        logger.info("Merging due to organelle(s) %s at level=%s", mer, self.level)
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------ 
  def _fission(self,tensor,filtering = [1.5,0]):
    ...
    if len(div) in [0,len(self.cell.organelles)]:
      logger.warning("SuperCell._fission: total fission canceled at level %s", self.level)
    else:
      self.divide_base(div,tensor)
      logger.info("Division occurring due to organelle(s) %s at level=%s", div, self.level)
#------------------------------------------------------------------------------

...
class Cell(object):
  def __init__(self,dimension,residual,cytosol,organelles):
    ...
    if not(valid):
      _raise_error("Error: in Cell.__init__: list dimensions do not equal "+ str(dimension))
    #~~~~~~~~~~~~~~~~~~~  
...
#-----------------------------------------------------------------  
  def copy(self):
    organelles = [x[:] for x in self.organelles]
...
#-----------------------------------------------------------------  
  def compose(self,index,a_cell):
    ...
    if self.dimension != a_cell.dimension:
      _raise_error("Error: in Cell.factorize: dimensions are not equal")
    #~~~~~~~~~~~~~~~~~~~ 
...
#-----------------------------------------------------------------  
  def compose_all(self,cells):
    ...
    if len(cells) != len(self.organelles):
      _raise_error(f"Error: in simultaneously_compose: input needs {len(self.organelles)} cells")
    #~~~~~~~~~~~~~~~~~~~
    the_cell = self
    composition_index = 0
...
#-----------------------------------------------------------------
  def left(self,alpha_var,kappa_var,lambda_var):
    ...
    if not(len(alpha_var) == len(kappa_var) == len(lambda_var)):
      _raise_error("Error: in Cell.left: dimensions of variables are not equal")
    #~~~~~~~~~~~~~~~~~~~
...
#-----------------------------------------------------------------  
  def right(self,mu_var,alpha_var,kappa_var):
    ...
    and len(mu_var) == len(alpha_var) == len(kappa_var)):
      _raise_error(
        f"Error: in Cell.right: tilling exceed the number of organelles ({sum(mu_var)} vs {len(self.organelles)})"
      )
    #~~~~~~~~~~~~~~~~~~~
    output = list()
    L = 0
...
#-----------------------------------------------------------------
  def spontaneous_reaction(self):
    ...
    if self.residual < 0:
      self.stdout()
      _raise_error("Error: in Cell.spontaneous_reaction: self.residual is not positive")
    #~~~~~~~~~~~~~~~~~~~ 
    for u in range(self.dimension):
...
#-----------------------------------------------------------------
  def action(self,matrix):
    ...
    if len(matrix) != len(self.organelles):
      self.stdout()
      _raise_error("Error: in Cell.action: the input list should contain "+str(len(self.organelles))+" list(s)")
    #~~~~~~~~~~~~~~~~~~~ 
    ...
    if self.SK != 0:
      for i in range(len(self.organelles)):
        ...
        if len(matrix[i]) != self.dimension:
          self.stdout()
          _raise_error(
            "Error: in Cell.action: the lists of the input should contain "+str(self.dimension)+" float items"
          )
        #~~~~~~~~~~~~~~~~~~~ 
        for u in range(self.dimension):
...
#------------------------------------------------------------------------------
  def algebra_operator(self,matrix):
    ...
    if len(matrix) != len(self.organelles):
      self.stdout()
      _raise_error("Error: in Cell.algebra_operator: the input list should contain "+str(len(self.organelles))+" list(s)")
    #~~~~~~~~~~~~~~~~~~~ 
      ...
      for u in range(self.dimension):
        for k in range(len(self.organelles)):
          ...
          if len(matrix[k]) != self.dimension:
            self.stdout()
            _raise_error(
              "Error: in Cell.algebra_operator: the lists of the input should contain "+str(self.dimension)+" float items"
            )
          #~~~~~~~~~~~~~~~~~~~ 
...
#-----------------------------------------------------------------   
  def allostasis(self,matrix,weight,org_index,dim_index):
    ...
    if len(matrix) != len(self.organelles) or len(matrix[org_index]) != self.dimension:
      self.stdout()
      _raise_error("Error: in Cell.allostasis: the size of the first input should be "+str(len(self.organelles))+"x"+str(self.dimension)+"-list")
    #~~~~~~~~~~~~~~~~~~~ 
    diff = 0
    if self.SK != 0:
      for u in range(self.dimension):
...
#------------------------------------------------------------------------------
  def divide(self,list_of_organelles,order="non-sorted"):
    ...
    if self.cytosol != [0] * self.dimension:
      _raise_error("Error: in Cell.divide: cytosol is not zero -- cannot divide")
    #~~~~~~~~~~~~~~~~~~~
...
#------------------------------------------------------------------------------
  def organelle_proportions(self):
    prop = list()
    for i in range(len(self.organelles)):
      prop.append([float(x)/self.Sorg[i] if self.Sorg[i]!=0 else 0 for x in self.organelles[i]])
    return prop
#------------------------------------------------------------------------------
  def content_proportions(self):
    s = sum(self.Sorg)
    prop = [float(x)/s if s!=0 else 0 for x in self.Sorg]
    return prop
#------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------      
  def proposed_clustering(self,matrix,option,filtering = [1.5,0]):
    ...
    if len(matrix) != len(self.organelles):
      self.stdout()
      _raise_error(
        "Error: in Cell.proposed_clustering: the input list should contain " + \
      ...
      if self.SK != 0:
        for i in range(len(self.organelles)):
          ...
          if len(matrix[i]) != self.dimension:
            self.stdout()
            _raise_error(
              "Error: in Cell.proposed_clustering: the "+str(i)+ \

...
class _Useful:
  ...
  def print_data(self,inputs,image_size,output_file,option = 'display'):
    ...
    if option == 'save':
      output_file.write("\t".join([str(int(100000*x)/100000.0) for x in inputs]))
  ...
  def get_trees(self,fload,cycles):
    ...
    for i, s in enumerate(fload):
      ...
      if s[0] == '@':
        info = list(map(int,self.parse_image(s,['@',':'],['\n'])))
  ...
  def rgb_colormap(self,maximum,grayscale = False,residual_heat = 1.0): 
    ...
    return lambda x : list(map(rescale,x)) if len(x) != 1 else contrast(x)
  #------------------------------------------------------------------------------

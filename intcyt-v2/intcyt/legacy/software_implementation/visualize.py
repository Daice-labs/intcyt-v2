...
def set_legends(fig,ax,image_size,query,ysubtitle,title):
  ...
  numlabel = [image_size[0]/2+(x)*(image_size[0]+1) for x in range(len(query))]
  ax.yaxis.set_major_locator(ticker.FixedLocator(numlabel))
  labels = [x for x in list(sorted(query))]
...
def visualize(fload,inputs,batch,query,image_size,title):
  ...
  extension = usf.make_rgb_grid([[x] for x in inputs],image_size,grayscale = True)
...
for i in range((len(sys.argv)-2)/2):
  query = query + list(range(int(sys.argv[2*i+r]),int(sys.argv[2*i+r+1]))) 
#-------------------------------------------------------------------------------------------
...
if sys.argv[1] == "SVHN":
  ...
  query_by_epoch = [x/epoch for x in query]
  collect_inputs = list()
...
if sys.argv[1] == "DREAM3":
  image_size = [8,8,1] #bicolor
  batch_roo = 1
  batch_org = 50 #same as ary
  epoch = 4
  training_images = 9235
  
  dim = image_size[0] * image_size[1] * image_size[2]
  fim = gzip.open("data/dream3_training.gz","r")
  listim = usf.get_memory(fim,1,list(range(training_images)))

...
class Data(object):
  def __init__(self,fdata,fchal,fansw):
    ...
    for i in range(len(self.answer)):
      self.answer[i] = list(map(int,self.answer[i]))
#---------------------------------------------------------------------------------------
      ...
      for i in range(len(data.genes)):
        ...
        if data.genes[i] in targets[k][j]:
          ...
          for s in range(len(strains)):
            e.extend([function(data.expressions[i][x+strains[s]*8]) for x in times]) 
          expr.append(e)
...
def contrast_profiles(brightness,vector):
  ...
  percents = [str(int(10000.*x)/10000.) for x in percents]
...
def distribution(vector,subdivisions,option = "pdf"):
  ...
  for i in range(len(cumul)):
    if cumul[i] == 0:
      ...
      if count == 1:
        threshold = values[i]
  return (list(range(subdivisions)),cumul,colors,threshold)
#---------------------------------------------------------------------------------------
def plot_bar(values,color,tittle,xlabel,xticks,ylabel,precision = lambda t : int(100*t)/100.0):
  fig, ax = plt.subplots(1)
  barplot = ax.bar(list(range(len(values))),values, color = color)
  #plt.bar(x, y)
  for bar in barplot:
    yval = bar.get_height()
    #print yval
...
def plot_bar2(new_integration,vertical,horizontal,interval,color,option = "raw",precision = lambda t : int(100*t)/100.0):
  ...
  for j in range(vertical):
    for k in range(horizontal):
      ...
      #------------------------------------
      ax[j][k].bar(list(range(len(pdf))),pdf, color = color, alpha = 0.8)
      xticks = [str(int(100*t)/100.0) for t in val]
      plt.sca(ax[j, k])
      plt.xticks(list(range(len(xticks))), xticks)
      ...
      if option == "approximate" or option == "model":
        ...
        ax[j][k].plot(list(range(len(pdf))),smooth,c = "purple")
      if option == "model":
        h     = max(smooth)
        mu    = index_interval(val,val[smooth.index(h)])
        sigma = abs((mu - index_interval(val,.9))/3.0)
        c = scstat.norm.pdf(list(range(len(smooth))),mu,sigma)
        c = [h/float(max(c)) *i for i in c]
        plt.plot(list(range(len(smooth))),c)
  #------------------------------------    
...
def space(height,width):
  ...
  return sp
#---------------------------------------------------------------------------------------
def load_dream3_data(challenge_dir: Path) -> Data:
  data_path = challenge_dir / "ExpressionData_UPDATED.txt"
  target_path = challenge_dir / "TargetList.txt"
  answer_path = challenge_dir / "ExpressionChallenge.txt"
  with data_path.open("r", encoding="utf-8") as fdata, \
       target_path.open("r", encoding="utf-8") as fchal, \
       answer_path.open("r", encoding="utf-8") as fansw:
    return Data(fdata,fchal,fansw)
#---------------------------------------------------------------------------------------
def command_gauss():
  plt.plot(list(range(10)),scstat.norm.pdf([t/10.0 for t in range(10)],.9,.3))
  plt.show()
#---------------------------------------------------------------------------------------
def command_figure1(data: Data):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[0,1,2,3])
  ...
  plt.show()
#---------------------------------------------------------------------------------------
def command_figure2(data: Data):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[0,2,3])
  ...
  load_initial = get_targets_expressions(queries,data,list(range(0,8)),[1,0,2,3])[0]
  ...
  plt.show()
#---------------------------------------------------------------------------------------
def command_figure3(data: Data):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[1,0,2,3])
  ...
  plt.show()
#---------------------------------------------------------------------------------------
def command_easy_answer(data: Data):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[0,2,3])
  model = get_targets_expressions([data.antitargets],data,list(range(0,8)),[1])[0]
  print("load")
  train = list()
  ...
  fig, ax = plt.subplots(1)
  ax.plot(list(range(len(corr))), corr, '-o', color='black')
  ax.plot(list(range(len(corr))), [average_corr_gene]*len(corr), color='red')
  plt.show()
  fig, ax = plt.subplots(1)
  ax.plot(list(range(len(pval))), pval, '-o', color='gray')
  print("-log(pval) = ", [int(-100*math.log10(t))/100.0 for t in pval])
  plt.show()
  ...
  fig, ax = plt.subplots(1)
  ax.plot(list(range(len(corr))), corr, '-o', color='black')
  ax.plot(list(range(len(corr))), [average_corr_time]*len(corr), color='red')
  plt.show()
  fig, ax = plt.subplots(1)
  ax.plot(list(range(len(pval))), pval, '-o', color='gray')
  print("-log(pval) = ", [int(-100*math.log10(t))/100.0 for t in pval])
...
#---------------------------------------------------------------------------------------
def command_method(data: Data):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[0,2,3])
  train = [separate_up_down(expr, lambda x : 100*x) for expr in training]
  memor = [separate_up_down(expr, lambda x : 100*x) for expr in load_initial]
  intensities = list()
  for i, memor_i in enumerate(memor):
    row = list()
    for j in range(len(train)):
  ...
  ax.scatter(list(range(len(integration))),integration,s=5, color = "red")
  fig, ax = plt.subplots(1)
  if agree_graph:
  ...
  ax.scatter(x,agree_graph,s=2, color = "black")
  plt.show()
  new_integration = sorted(integration)
  if new_integration:
    M = max(new_integration)
    m = min(new_integration)
  else:
    M = m = 0
  I = (M-m)/5.0 if M != m else 0
  fig, ax = plt.subplots(1)
  ax.scatter(x,agree_graph,s=2, color = "gray")
  ax.scatter(list(range(len(integration))),integration,s=5, color = "red")
  for k in range(6):
    plt.hlines(m+k*I,0,max(len(integration)-1,0), color = "#58cd4c")
  plt.show()
  ...
  plt.show()
#---------------------------------------------------------------------------------------
def command_training(data: Data, training_output: Path, load_output: Path):
  queries = [data.antitargets,data.targets]
  training, load_initial = get_targets_expressions(queries,data,list(range(0,8)),[1,0,2,3])
  training_output.parent.mkdir(parents=True, exist_ok=True)
  load_output.parent.mkdir(parents=True, exist_ok=True)
  with gzip.open(training_output,"wt", encoding="utf-8") as ftrain:
    for expr in training:
      train_i = separate_up_down(expr, lambda x : 100*x)
      ftrain.write("\t".join(map(str,train_i))+"\n")
  image = list()
  with gzip.open(load_output,"wt", encoding="utf-8") as fmemor:
    for expr in load_initial:
      memor_i = separate_up_down(expr, lambda x : 100*x)
...
#---------------------------------------------------------------------------------------
def command_result(data: Data, save_path: Path, cycle: Optional[int], interval: int):
  ary = 50
  if cycle is None:
    with gzip.open(save_path,"rt", encoding="utf-8") as fload:
      cycle = usf.get_last_cycle(fload,ary)
    print(f"loading organelles at index {cycle} from {save_path.name}")
  index_history = [interval*i for i in range(int(cycle/float(interval)))]
  if not index_history or index_history[-1] != cycle:
    index_history.append(cycle)
  with gzip.open(save_path,"rt", encoding="utf-8") as fload:
    list_of_organelles = usf.get_memory(fload,ary,index_history)
  recovery_of_memory = get_targets_expressions([data.targets],data,list(range(0,8)),[0,2,3],float)[0]
  list_of_scores = list()
  for k, organelles in enumerate(list_of_organelles):
    expression = list()
    for org in organelles:
      r = reunite_up_down(org[:16], lambda t: t/100.0)
      x = reunite_up_down(org[16:], lambda t: t/100.0)
      expression.append(r+x)
    ordered_expression = list()
    for i in range(len(expression)):
      agree_list = list()
      for j in range(len(recovery_of_memory)):
        agree = agreement(expression[i],recovery_of_memory[j])
        agree_list.append([agree,j])
      agree_list = sorted(agree_list, key = lambda x:-x[0])
      ordered_expression.append([i,agree_list[0][1],agree_list[0][0]])
    ordered_expression = sorted(ordered_expression, key = lambda x:-x[2])
    matrix = list()
    for i in range(len(ordered_expression)):
      matrix.append(recovery_of_memory[ordered_expression[i][1]])
    ...
    print("\033[91mscore = ", list_of_scores[-1], "\033[0m")
    if k == len(list_of_organelles)-1:
      try:
        ...
        plt.show()
        fig, ax = plt.subplots(1)
        ax.plot(list(range(len(corr_gene))), corr_gene, '-o', color='black')
        ax.plot(list(range(len(corr_gene))), [average_corr_gene]*len(corr_gene), color='red')
        plt.show()
        fig, ax = plt.subplots(1)
        ax.plot(list(range(len(pval_gene))), pval_gene, '-o', color='gray')
        print("-log(pval_gene) = ", [int(-100*math.log10(t))/100.0 for t in pval_gene])
        plt.show()
        fig, ax = plt.subplots(1)
        ax.plot(list(range(len(corr_time))), corr_time, '-o', color='black')
        ax.plot(list(range(len(corr_time))), [average_corr_time]*len(corr_time), color='red')
        plt.show()
        fig, ax = plt.subplots(1)
        ax.plot(list(range(len(pval_time))), pval_time, '-o', color='gray')
        print("-log(pval_time) = ", [int(-100*math.log10(t))/100.0 for t in pval_time])
        plt.show()
      except Exception:
        pass
  colors = list()
  for i in range(int(cycle/float(interval))+1):
    if i in [int(1000*x/float(interval)) for x in [36,37,38,73,74,75,119,120,121]]:
      colors.append("red")
    else:
      colors.append("black")
  fig, ax = plt.subplots(1)
  x = [interval * s for s in range(int(cycle/float(interval)))] + [cycle]
  ax.scatter(x, list_of_scores, s = 5, color=colors)
  plt.show()
#---------------------------------------------------------------------------------------
def build_parser() -> argparse.ArgumentParser:
  parser = argparse.ArgumentParser(
    description="Utilities for reproducing the DREAM3 figures and analyses from the legacy IntCyt release.",
  )
  parser.add_argument(
    "--challenge-dir",
    type=Path,
    default=Path("data") / "expression_challenge",
    help="Directory containing the ExpressionData/TargetList/ExpressionChallenge files.",
  )
  subparsers = parser.add_subparsers(dest="command", required=True)

  subparsers.add_parser("gauss", help="Plot the Gaussian curve used in Figure 2.8.")
  subparsers.add_parser("figure1", help="Recreate Figure 1 from the documentation.")
  subparsers.add_parser("figure2", help="Recreate Figure 2 from the documentation.")
  subparsers.add_parser("figure3", help="Recreate Figure 3 from the documentation.")
  subparsers.add_parser("easy-answer", help="Run the cosine-similarity baseline.")
  subparsers.add_parser("method", help="Run the DREAM3 method analysis (Section 2.8.4).")

  training_parser = subparsers.add_parser("training", help="Generate dream3_training.gz and load_initial.gz.")
  training_parser.add_argument(
    "--training-output",
    type=Path,
    default=Path("data") / "dream3_training.gz",
    help="Destination for the generated training gzip file.",
  )
  training_parser.add_argument(
    "--load-output",
    type=Path,
    default=Path("result-load") / "load_initial.gz",
    help="Destination for the generated initialization gzip file.",
  )

  result_parser = subparsers.add_parser("result", help="Visualize saved organelles over time.")
  result_parser.add_argument(
    "cycle",
    nargs="?",
    type=int,
    help="Specific cycle to load (defaults to the last cycle present).",
  )
  result_parser.add_argument(
    "--interval",
    type=int,
    default=1000,
    help="Sampling interval when replaying the saved organelles.",
  )
  result_parser.add_argument(
    "--save-org",
    type=Path,
    default=Path("save_org.gz"),
    help="Path to the gzip file produced by main.py (default: save_org.gz).",
  )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
  ...
  data = load_dream3_data(args.challenge_dir)

  if args.command == "figure1":
    command_figure1(data)
  elif args.command == "figure2":
    command_figure2(data)
  elif args.command == "figure3":
    command_figure3(data)
  elif args.command == "easy-answer":
    command_easy_answer(data)
  elif args.command == "method":
    command_method(data)
  elif args.command == "training":
    command_training(data, args.training_output, args.load_output)
  elif args.command == "result":
    command_result(data, args.save_org, args.cycle, args.interval)
  else:
...
if "result" in sys.argv[1:]:
  ...
  for k in range(len(list_of_organelles)):
    ...
    try:
      if k == len(list_of_organelles)-1:
        ...
        print("\n----------\n")
        for i in range(len(organelles)):
          print(i, [int(1000000*x) for x in matrix[i]], "...", int(-100*math.log10(pval_time[i]))/100.0)
        print("\n----------\n")
        for i in range(len(organelles)):
      ...
      if k == len(list_of_organelles)-1:
        ...
        print("\n----------\n")
        for i in range(len(organelles)):
          print(i, [int(1000000*x) for x in matrix[i]])
        print("\n----------\n")
        for i in range(len(organelles)):
  ...
  fig, ax = plt.subplots(1)
  x = [interval * s for s in range(int(load_index/float(interval)))] + [load_index]
  ax.scatter(x, list_of_scores, s = 5, color=colors)
  plt.show()  
#---------------------------------------------------------------------------------------

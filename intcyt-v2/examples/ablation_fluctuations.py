"""Structural/Fluctuation ablation experiments for Intcyt on MNIST / Fashion-MNIST.

This script compares the baseline Intcyt training loop against ablations that
remove internal structural fluctuations during learning.

Conditions
----------
1) **full**: default schedule (fission + fusion + compose enabled).
2) **no_fluctuations**: fission and fusion disabled, compose kept enabled.
3) **no_struct_ops**: fission, fusion, and compose all disabled (numeric updates only).

The evaluation protocol is identical to ``examples/evaluate_test_recall.py``:
- train Intcyt without labels
- build a majority-vote mapping from learned prototypes -> labels on the train set
- compute test micro/macro recall

Usage
-----
  python examples/ablation_fluctuations.py --dataset mnist --runs 5
  python examples/ablation_fluctuations.py --dataset fashion-mnist --runs 5
  python examples/ablation_fluctuations.py --dataset both --runs 5
...
# Reuse the exact same dataset loader + training/eval helpers.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Condition tuples: (name, fission_events, fusion_events, compose_events)
    conditions: List[Tuple[str, Tuple[int, ...], Tuple[int, ...], Tuple[int, ...]]] = [
        ("full", (0,), (2,), (1, 3)),
        ("no_fluctuations", (), (), (1, 3)),
        ("no_struct_ops", (), (), ()),
    ]
    ...
    for ds in datasets:
        ...
        for cond_name, fission_events, fusion_events, compose_events in conditions:
            micro_vals: List[float] = []
            macro_vals: List[float] = []
            runs: List[Dict[str, object]] = []

            print(
                f"\n--- Condition: {cond_name} (fission={list(fission_events)}, fusion={list(fusion_events)}, compose={list(compose_events)}) ---"
            )
            ...
            ds_block[cond_name] = {
                "events": {
                    "fission": list(fission_events),
                    "fusion": list(fusion_events),
                    "compose": list(compose_events),
                },
        ...
        # Convenience: one-line comparison table for this dataset.
        print("\n=== Comparison (mean ± sd) ===")
        for cond_name, _, _, _ in conditions:
            summ = ds_block[cond_name]["summary"]
            print(
                f"{cond_name:>14s}: micro {float(summ['micro_recall_mean']) * 100:.2f}% ± {float(summ['micro_recall_sd']) * 100:.2f}%"
                f" | macro {float(summ['macro_recall_mean']) * 100:.2f}% ± {float(summ['macro_recall_sd']) * 100:.2f}%"
            )
